...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('assets/vendor/apexcharts/css/apexcharts.css') }}" type="text/css">
    @yield('css')
...
<body>
    <main>
        @include('laundry.admin.layouts.sidebar')

		<div class="page-content">
            @include('laundry.admin.layouts.header')
    ...
    <script>
        function markAsReadAndNavigate(notificationId) {
            console.log('ID Pemberitahuan:', notificationId);
            ...
            $.ajax({
                ...
                success: function(response) {
                    console.log('Notifikasi yang ditandai sebagai telah dibaca:', response);
                    window.location.href = '{{ route("admin.review") }}';
                },
                error: function(error) {
                    console.error('Pemberitahuan kesalahan menandai telah dibaca:', error);
                }
            });
        }

        function markAllAsRead() {
            console.log('Tandai semua notifikasi sebagai telah dibaca');

            $.ajax({
                type: 'POST',
                url: '{{ route("admin.markAllAsRead") }}',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                success: function(response) {
                    console.log('Semua notifikasi yang ditandai sebagai telah dibaca:', response);
                    location.reload();
                },
                error: function(error) {
                    console.error('Kesalahan menandai semua notifikasi sebagai telah dibaca:', error);
                }
            });
        }
    </script>
