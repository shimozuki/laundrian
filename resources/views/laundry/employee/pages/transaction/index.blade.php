    ...
    <div class="row">
        <div class="col-12">
            <div class="card border rounded-0 h-100">
                <div class="card-body">
                    <form method="GET" action="{{ route('employee.transaction') }}" class="d-flex justify-content-between mb-3">
                                                ...
                                                <div class="ms-4">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end min-w-auto border rounded-0" aria-labelledby="actionDropdown">
                                                        @if ($row->status == 'pending')
                                                            <li>
                                                                <form method="POST" action="{{ route('employee.transactionProcessed', $row->invoice) }}">
                                                                    @csrf
                                                        ...
                                                        @elseif ($row->status == 'processed')
                                                            <li>
                                                                <form method="POST" action="{{ route('employee.transactionComplete', $row->invoice) }}">
                                                                    @csrf
                                                                    <button type="submit" class="dropdown-item bg-dark-soft-hover">
                                                        ...
                                                        @elseif ($row->status == 'completed')
                                                            <li>
                                                                <form method="POST" action="{{ route('employee.transactionRetrieved', $row->invoice) }}">
                                                                    @csrf
                                                                    <button type="submit" class="dropdown-item bg-dark-soft-hover">
                                                        ...
                                                        <li>
                                                            <a href="{{ route('employee.transactionReceipt', $row->invoice) }}" class="dropdown-item" target="_blank">
                                                                <i class="fa-sharp fa-regular fa-receipt me-2"></i>Struk
                                                            </a>
                                                        </li>
                                                        <li>
                                                            <form method="POST" id="delete-form-{{ $row->invoice }}" action="{{ route('employee.transactionDestroy', $row->invoice) }}">
                                                                @csrf @method('DELETE')
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            $('#customer_id').change(function() {
                ...
                if (customerId) {
                    $.ajax({
                        url: '{{ url('/employee/transaction') }}/' + customerId,
                        type: 'GET',
                        dataType: 'json',
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (invoice) {
                var printUrl = '{{ route('employee.transactionReceipt', ['invoice' => '__invoice__']) }}'.replace('__invoice__', invoice);
    ...
    <div class="modal fade" id="transactionModal" tabindex="-1" aria-labelledby="transactionModallabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-md">
            <div class="modal-content border rounded-0">
                ...
                <div class="modal-body">
                    <form method="POST" action="{{ route('employee.transactionStore') }}" enctype="multipart/form-data">
                        @csrf
    ...
    <div class="modal fade" id="customerModal" tabindex="-1" aria-labelledby="customerModallabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-md">
            <div class="modal-content border rounded-0">
                ...
                <div class="modal-body">
                    <form method="POST" action="{{ route('employee.transactionStoreCustomer') }}" enctype="multipart/form-data">
                        @csrf
                        <div class="row px-xxl-2">
                            <div class="col-12">
