<nav class="navbar top-bar navbar-expand-xl bg-success navbar-dark shadow-sm py-0 py-xl-3">
            ...
            <ul class="nav flex-row align-items-center list-unstyled ms-xl-auto">
                    ...
                    <div class="dropdown-menu dropdown-animation dropdown-menu-end dropdown-menu-size-md shadow-lg p-0">
                        <div class="card bg-transparent">
                            <div class="card-header bg-transparent d-flex justify-content-between align-items-center border-bottom">
                                @if($unreadCount > 0)
                                <h6 class="m-0">Pemberitahuan <span class="badge bg-dark rounded-0 bg-opacity-10 text-dark ms-2">{{ $unreadCount }} baru</span></h6>
                                @else
                                <h6 class="m-0">Pemberitahuan</h6>
                                @endif
                                @if($hasUnreadNotification)
                                <a href="javascript:;" class="fw-normal small text-dark" onclick="markAllAsRead()">Hapus Semua</a>
                                @endif
                            </div>
                                    ...
                                    <ul class="list-group list-group-flush list-unstyled p-2">
                                        ...
                                        <li>
                                            <div class="list-group-item list-group-item-action rounded-0 notif-unread text-center border-0 mb-1 p-3">
                                                <span class="fw-normal smaller h6 mb-3">Tidak ada pemberitahuan pesan baru</span>
                                            </div>
                                        </li>
                                        @else
                                        @foreach($notifications as $notification)
                                        <li>
                                            <a href="javascript:;" class="list-group-item list-group-item-action rounded-0 notif-unread border-0 mb-1 p-3" onclick="markAsReadAndNavigate('{{ $notification->notification_id }}')">
                                                <div class="d-md-flex">
                                                    <img src="{{ asset('storage/profiles/' . ($notification->customer_image ?? 'avatar.jpg')) }}" alt="avatar" class="avatar avatar-sm rounded-circle me-3">
                                                    <div class="mt-0">
                                                        <h6 class="mb-0">{{ $notification->customer_name }}</h6>
                                                        <p class="mb-0">{{ $notification->comment }}</p>
                                                    </div>
                                                </div>
                                            </a>
                                        </li>
                ...
                <li class="nav-item ms-3 dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end dropdown-animation rounded-0 pt-3" aria-labelledby="profileDropdown">
                        ...
                        <li>
                            <a href="{{ route('admin.profile', Auth::user()->username) }}" class="dropdown-item @yield('active-my-profile')">
